...
LABEL maintainer="Prabath Thalangama - https://sysadmin.lk"
...
RUN buildDeps='curl gcc make autoconf libc-dev zlib1g-dev pkg-config' \
    && set -x \
    && apt-get update \
    && apt-get install --no-install-recommends $buildDeps --no-install-suggests -q -y npm gnupg2 dirmngr wget apt-transport-https lsb-release ca-certificates \
    && \
